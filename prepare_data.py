...
7. **Merge new data into existing manifest** (preserves train/valid splits)

Usage:
    # Fresh manifest from 10% of data
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv --percentage 10

    # Expand existing manifest from 10% → 50% (NO mixing of existing splits)
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv \
        --merge-existing processed/manifest.csv --percentage 50

    # Expand to 100%
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv \
        --merge-existing processed/manifest.csv
"""
...
def load_input_data(input_path: Path, audio_dir: Optional[Path] = None) -> pd.DataFrame:
    ...
    # Remove any existing split column — we manage splits ourselves
...
def load_existing_manifest(manifest_path: Path) -> pd.DataFrame:
    """
    Load an existing manifest CSV that already has train/valid splits.

    Returns DataFrame with columns: id, sentence, split
    """
    df = pd.read_csv(manifest_path)

    required_cols = {'id', 'sentence', 'split'}
    if not required_cols.issubset(df.columns):
        missing = required_cols - set(df.columns)
        raise ValueError(
            f"Existing manifest is missing columns: {missing}. "
    ...
    print(f"  Loaded existing manifest: {len(df):,} samples "
          f"(train={train_count:,}, valid={valid_count:,})")

    return df[['id', 'sentence', 'split']]
...
def merge_with_existing(
    ...
    """
    Merge new data into an existing manifest without mixing train/valid.

    Strategy:
    - Keep all existing samples with their original train/valid labels
    - From new_df, only take samples whose IDs are NOT in existing_df
    - Split the truly-new samples into train/valid using valid_ratio
    - Concatenate and return

    This guarantees:
    - No sample that was in valid moves to train (or vice versa)
    - No data leakage between splits
    - The new valid_ratio only applies to the *new* samples being added
    """
    existing_ids = set(existing_df['id'].tolist())
    original_new_count = len(new_df)

    # Only keep samples that don't already exist in the manifest
    truly_new = new_df[~new_df['id'].isin(existing_ids)].copy().reset_index(drop=True)
    already_present = original_new_count - len(truly_new)

    print(f"\n  Merge summary:")
    print(f"    Existing manifest:    {len(existing_df):,} samples")
    print(f"    New data total:       {original_new_count:,} samples")
    print(f"    Already in manifest:  {already_present:,} (skipped)")
    print(f"    Genuinely new:        {len(truly_new):,} samples to add")

    if len(truly_new) == 0:
        print("  ⚠️  No new samples to add — all IDs already exist in manifest.")
        return existing_df

    # Split the genuinely new samples into train/valid
    truly_new_split = split_dataset(
        truly_new,
        valid_ratio=valid_ratio,
        stratify_by_length=stratify_by_length,
        seed=seed
    )

    new_train = (truly_new_split['split'] == 'train').sum()
    new_valid = (truly_new_split['split'] == 'valid').sum()
    print(f"    New samples split:    train={new_train:,}, valid={new_valid:,}")

    # Concatenate existing (with their original splits) + new
    merged = pd.concat([existing_df, truly_new_split], ignore_index=True)

    # Final counts
    total_train = (merged['split'] == 'train').sum()
    total_valid = (merged['split'] == 'valid').sum()
    print(f"\n  ✅ Final merged manifest: {len(merged):,} samples "
          f"(train={total_train:,}, valid={total_valid:,})")
...
def limit_dataset(
    ...
    Args:
        ...
        percentage: Percentage of data to use (e.g., 50 for 50%)
...
Examples:
    ...
    # Expand existing 10% manifest → 50% (preserves existing train/valid labels)
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv \\
        --merge-existing processed/manifest.csv --percentage 50

    # Expand existing manifest → 100% (no --percentage means use all)
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv \\
        --merge-existing processed/manifest.csv

    # Expand to 100% and validate audio files exist
    python prepare_data.py --input raw_data.csv --output processed/manifest.csv \\
        --merge-existing processed/manifest.csv --audio-dir data/train --validate
        """
    )

    parser.add_argument('--input', '-i', type=str, required=True,
                        help='Input CSV/JSON file with all transcriptions (full dataset)')
    parser.add_argument('--output', '-o', type=str, required=True,
                        help='Output CSV path (e.g., processed/manifest.csv)')
    parser.add_argument('--audio-dir', '-a', type=str, default=None,
                        help='Audio directory to validate files exist')

    # Merge option
    merge_group = parser.add_argument_group('Merge with Existing Manifest')
    merge_group.add_argument('--merge-existing', '-m', type=str, default=None,
                             help='Path to existing manifest CSV to merge into. '
                                  'Existing train/valid splits are preserved. '
                                  'Only new IDs (not already in manifest) are added.')

    # Data limiting options
    limit_group = parser.add_argument_group('Data Limiting')
    limit_group.add_argument('--limit', '-l', type=int, default=None,
                             help='Maximum total samples to use from input (e.g., 50000)')
    limit_group.add_argument('--percentage', '-p', type=float, default=None,
                             help='Percentage of input data to use (e.g., 50 for 50%%)')

    # Split options
    split_group = parser.add_argument_group('Train/Valid Split')
    split_group.add_argument('--valid-ratio', '-v', type=float, default=0.1,
                             help='Validation ratio for NEW samples only (default: 0.1)')
    ...
    if merge_path:
        if not merge_path.exists():
            raise FileNotFoundError(f"Existing manifest not found: {merge_path}")
        print(f"\n📂 Loading existing manifest: {merge_path}")
        existing_df = load_existing_manifest(merge_path)
    ...
    # ---------------------------------------------------------------
    # Either MERGE into existing or do a fresh split
    # ---------------------------------------------------------------
    if existing_df is not None:
        print(f"\n🔀 Merging new data into existing manifest...")
        final_df = merge_with_existing(
            new_df=df,
        ...
        print(f"\n✂️  Fresh split (valid_ratio={args.valid_ratio})")
        final_df = split_dataset(
            df,
            valid_ratio=args.valid_ratio,
            stratify_by_length=args.stratify,
            seed=args.seed
    ...
    # Compute statistics
    stats = compute_text_stats(final_df)

    # Print info
    if not args.quiet:
        print_stats(final_df, stats)
        print_samples(final_df)
    ...
    valid_count = len(final_df[final_df['split'] == 'valid'])
    print(f"\n{'='*60}")
    print(f"✅ Done! Train: {train_count:,} | Valid: {valid_count:,} | Total: {len(final_df):,}")
    print(f"{'='*60}")
